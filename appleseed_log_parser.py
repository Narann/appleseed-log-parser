...
# Prepare regex
################################################################################
main_regex  = '(?P<timestamp>[0-9TZ:.-]+)\s+'
main_regex += '\<(?P<thread_id>\d{3})\>\s+'
main_regex += '(?P<vm>\d+)\s+MB\s+'
main_regex += '(?P<msg_cat>\w+)\s+'
main_regex += '\|(?P<msg_rest>.*)'
re_main = re.compile( main_regex )
...
loaded_mesh_file_regex  = '\s*loaded mesh file (?P<mesh_path>[\w./\\\\-]+) '
loaded_mesh_file_regex += '\((?P<objects>[\d,]+) object, '
loaded_mesh_file_regex += '(?P<vertices>[\d,]+) vertices, '
loaded_mesh_file_regex += '(?P<triangles>[\d,]+) triangles\) '
loaded_mesh_file_regex += 'in (?P<milliseconds>[\d,]+) ms\.'
re_loaded_mesh_file     = re.compile( loaded_mesh_file_regex )
...
class ASLogParserUI( QtGui.QMainWindow ) :
    ...
    def refresh_recent_log_files_listWidget( self ) :
        found = False
        for i in range(self.recent_log_files_listWidget.count()):
            current_item = self.recent_log_files_listWidget.item( i )
            if current_item.as_log_file_path == self._current_log :
                self.recent_log_files_listWidget.setCurrentItem( current_item )
                found = True
                break
        if not found :
            label = self._current_log
            current_item = QtGui.QListWidgetItem( label )
            current_item.as_log_file_path = self._current_log
    ...
    def refresh_filtered_log_view( self ) :
        ...
        for line_data in current_log_data.lines_data :
            ...
            if self._show_icons :
                ...
                elif line_type in [ 'rendering_progress' ] :
                    icon = self.icons[ 'progress' ]
                elif line_type in [ 'opening_texture_file' ] :
